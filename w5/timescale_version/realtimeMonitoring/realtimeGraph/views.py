...
def get_user_map_json(request, **kwargs):
    ...
    for user in users:
        stations = Station.objects.filter(user=user)
        usersData = Data.objects.filter(
            station__in=stations, measurement__name=selectedMeasure.name,  time__gte=start_ts, time__lte=end_ts
        )        
        if usersData.count() <= 0:
            continue
        minVal = usersData.aggregate(Min("min_value"))["min_value__min"]
        maxVal = usersData.aggregate(Max("max_value"))["max_value__max"]
        avgVal = usersData.aggregate(Avg("avg_value"))["avg_value__avg"]
        data.append(
            {
                'name': f'{user.login}',
                "min": minVal if minVal != None else 0,
    ...
    data_result["users"] = [userValue.str() for userValue in users]
    data_result["start"] = startFormatted
    data_result["end"] = endFormatted
    data_result["data"] = data
